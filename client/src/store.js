...
const store = new Vuex.Store({
  ...
  actions: {
    initApp(context) {
      axios.get("http://localhost:8888/ci3apirestvue/api/get_all_data")
        .then(response => {
    ...
    addCourse(context, course) {
      // DB İşlemi...
      return axios.post("http://localhost:8888/ci3apirestvue/api/save", JSON.stringify(course))
        .then(response => {
    ...
    updateCourse(context, course) {
      // DB İşlemi...
      return axios.post("http://localhost:8888/ci3apire3stvue/api/update", JSON.stringify(course))
        .then(response => {
    ...
    deleteCourse(context, courseID) {
      // DB İşlemi...
      return axios.post("http://localhost:8888/ci3apirestvue/api/delete", JSON.stringify({ id : courseID}))
        .then(response => {
